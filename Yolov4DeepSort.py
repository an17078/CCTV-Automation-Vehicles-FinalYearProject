...
def main(_argv):
    ...
    model_filename = 'model_data/TrainedSystem.pb'
    ...
    region_1 = [(365, 400), (1015, 400), (1165, 445), (430, 445)]
    region_2 = [(685, 785), (1835, 785), (1912, 850), (710, 900)]
    ...
    while True:
        ...
        for track in tracker.tracks:
            ...
            cv2.putText(frame, class_name + "-" + 
                                str(track.track_id),(int(bbox[0]), int(bbox[1]-10)),0, 0.75, (255,255,255),2)
            cv2.polylines(frame, [np.array(region_1)], True, (0, 255, 255), 4)
            cv2.polylines(frame, [np.array(region_2)], True, (255, 0, 255), 4)

            if (class_name == "car" or class_name == "bus" or class_name == "truck" or 
                                        class_name == "bicycle" or class_name == "motorcycle"):
            ...
            if track.track_id in vehicle_entering_1:
                ...
                if result_1 >= 0:
                    ...
                    if avg_speed_kmh_1 > 85:
                        cv2.rectangle(frame, (int(bbox[0]), int(bbox[3])), 
                                (int(bbox[0]+105), int(bbox[3]+30)), color, -1)
                        cv2.putText(frame, "Speeding", (int(bbox[0]), 
                                int(bbox[3])+25), 0, 0.75, (255, 255, 255), 2)
                    else:
                        cv2.rectangle(frame, (int(bbox[0]), int(bbox[3])), 
                                (int(bbox[0]+165), int(bbox[3]+30)), color, -1)
                        cv2.putText(frame, "In speed limit", (int(bbox[0]), 
                                    int(bbox[3])+25), 0, 0.75, (255, 255, 255), 2)
                    cv2.rectangle(frame, (int(bbox[2]-95), int(bbox[3])), 
                                        (int(bbox[2]), int(bbox[3]+30)), color, -1)
                    cv2.rectangle(frame, (int(bbox[2])-95, int(bbox[3])), 
                                        (int(bbox[2]), int(bbox[3])+30), color, -1)
                    cv2.putText(frame, str(int(avg_speed_kmh_1)) + "km/h", 
                                        (int(bbox[2]-95), int(bbox[3])+25), 0, 0.75, (255, 255, 255), 2)
                ...
                if track.track_id in vehicle_entering_2:
                    ...
                    if result_2 >= 0:
                        ...
                        if avg_speed_kmh_2 > 85:
                            cv2.rectangle(frame, (int(bbox[0]), int(bbox[3])), (int(bbox[0]+105), int(bbox[3]+30)), color, -1)
                            cv2.putText(frame, "Speeding", (int(bbox[0]), int(bbox[3])+25), 0, 0.75, (255, 255, 255), 2)
                        else:
                            cv2.rectangle(frame, (int(bbox[0]), int(bbox[3])), (int(bbox[0]+165), int(bbox[3]+30)), color, -1)
                            cv2.putText(frame, "In speed limit", (int(bbox[0]), int(bbox[3])+25), 0, 0.75, (255, 255, 255), 2)
                        cv2.rectangle(frame, (int(bbox[2]-95), int(bbox[3])), (int(bbox[2]), int(bbox[3]+30)), color, -1)
                        cv2.putText(frame, str(int(avg_speed_kmh_2)) + "km/h", (int(bbox[2]-95), int(bbox[3])+25), 0, 0.75, (255, 255, 255), 2)
            ...
            print(region_2_ids)
            print(speed_2_list)
            cv2.rectangle(frame, (0, 0), (410, 95),(255,255,255), -1)
